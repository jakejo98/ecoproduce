...
function producerHorizontalScroll() {
  ...
  $(producerCont).scroll(function() {
    ...
    if(producerScrollLeft + producerWidth >= producerScrollWidth) {
      if(producerCount < 2) {
        producerCount++;
      // 스크롤 일시적으로 정지
      $(producerCont).addClass(isFixed);
      // 지정 시간 이후 리스트 추가로 보여줌
      setTimeout(function(){
        appendProducer();
        $(producerCont).removeClass(isFixed);
      }, 100);
      // 리스트 추가 후 변수 값 업데이트
...
// 기본 제공 생산자 스토리
function initAppendProducer(){
  const item = $('.section_story .producer_box .product_grid_item')
