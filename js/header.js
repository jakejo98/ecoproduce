...
/* Respond */
// 반응형 검색창
function headerSearchApp() {
  const searchApp = $('.header_search_app');
  const searchOpenBtn = $('.header_search_btn');
  const searchClsBtn = $('.header_search_app .close_btn');
  const searchCon = 'active';
  // 검색 버튼 클릭시 검색창 활성화
  $(searchOpenBtn).click(function(){
    $(searchApp).addClass(searchCon);
  })
  // 뒤로가기 버튼
  $(searchClsBtn).click(function(){
    $(searchApp).removeClass(searchCon);
  })
}
